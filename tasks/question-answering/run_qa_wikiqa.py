...
from utils_qa import Seq2SeqDataPreProcessor,\
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        raw_datasets = load_dataset(
            *TASK_MAPPING_DATASET_ARGUMENTS[data_args.dataset_name],
        )
    else:
        data_files = {}
    ...
    setattr(config, 'n_cluster', model_args.n_cluster)
    ...
    if training_args.do_train:
        ...
        with training_args.main_process_first(desc="train dataset map pre-processing"):
            train_dataset = train_dataset.map(
                Seq2SeqDataPreProcessor(benchmark=data_args.dataset_name),
                batched=True,
    ...
    if training_args.do_eval:
        ...
        with training_args.main_process_first(desc="validation dataset map pre-processing"):
            eval_examples = eval_examples.map(
                Seq2SeqDataPreProcessor(benchmark=data_args.dataset_name, keep_specific_keys=EXTRA_KEYS_FOR_EVAL),
                batched=True,
                num_proc=data_args.preprocessing_num_workers,
    ...
    def post_processing_function(
        ...
        label_mapping_id = get_label_mapping_id(data_args.dataset_name)
        eos_token_id = tokenizer.eos_token_id
        predictions = [p[:list(p).index(eos_token_id) + 1] if eos_token_id in p else p for
                       p in preds]
        ...
        predictions = [label_mapping_id[p] if p in label_mapping_id else random.choice(list(label_mapping_id.values()))
                       for p in predictions]
        references = [label_mapping_id[ex['targets']] for ex in examples]
